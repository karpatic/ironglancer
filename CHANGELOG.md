...
- Added source privacy modes: `none` default, `declarations`, and `full`, with explicit viewer/API/local-agent degradation when source text is unavailable.
...
- Added a Webpack plugin export that reruns normal IronGlancer generation after successful builds/rebuilds and manages the existing localhost viewer, read-only `/api/v1`, and `/bridge/v1` service without consuming Webpack's graph.
